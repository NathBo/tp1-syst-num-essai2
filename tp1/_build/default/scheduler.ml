...
let read_exp eq =
  ...
  let rec aux eq  acc = match eq with
    ...
    | Eram (_,_,a,b,c,d) -> auxarg a (auxarg b (auxarg c (auxarg d acc)))
...
let program_to_graph p =
  ...
  let rec add_exp l = match l with
    | [] -> ()
    | eq::q -> (add_node_list g (read_exp eq);add_exp q) in
  add_exp p.p_eqs;
  add_node_list g p.p_inputs;
  add_node_list g p.p_outputs;
  let rec add_eq l = match l with
    | [] -> ()
    | (i,expr)::q -> (add_edge_list g i (read_exp (i,expr));add_eq q) in
  add_eq p.p_eqs;g


let rec variables_to_eqs p_eqs l = match l with
  | [] -> []
  | v::q -> List.find (fun (a,b) -> a = v) p_eqs :: variables_to_eqs p_eqs q
